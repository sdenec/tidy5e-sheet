export const tidy5eShowItemArt = function (html, item) {
  ...
  image.attr(
    "title",
    `${game.i18n.localize("TIDY5E.EditActorImage")} / ${game.i18n.localize(
      "TIDY5E.ShowItemImage"
    )}`
  );
  let container = `<div class="item-image"></div>`;
  let menu = `<div class="item-menu hidden"><a class="showItemArt">${game.i18n.localize(
    "TIDY5E.ShowItemArt"
  )}</a>`;
  image.wrap(container);
  let imageContainer = html.find(".item-image");
